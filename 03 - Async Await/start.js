(() => {
  ...
  const similateAsyncAPI = (text, timeout) => { //เรียกใช้ฟังก์ชั่นหลังจาก Timeout 
    setTimeout(() => console.log(text), timeout);
  }

  similateAsyncAPI('A', 1000); //ผ่านไป 1 วิ จะ Console.log A ออกมา
  similateAsyncAPI('B', 500);
  similateAsyncAPI('C', 100);

  // 2. Callback //นิยมใน Node JS
  const similateAsyncAPI = (text, timeout, Callback) => { //callback จะเป็นฟังก์ชั่นที่ถูกเรียกใช้หลังจากฟังกชั่นของเราทำงานเสร็จแล้ว
    setTimeout(() => {
      console.log(text)
  ...
  // 3. Promise
  const similateAsyncAPI = (text, timeout) => { // concept ของ promise คือ ใช้  เรียกลงไปเรื่อยๆ
  ...
  // 4. Async/Await
  const similateAsyncAPI = (text, timeout) => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
